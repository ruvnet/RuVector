...
//!                      larger ones.
//! - `sparse_fiedler` — sparse eigensolve path for windows with more
//!                      than 1024 active neurons; uses
//!                      `ruvector_sparsifier::SparseGraph` as the
//!                      canonical scratch edge container so memory per
//!                      detect stays `O(n + nnz)` instead of `O(n²)`.
//! - `lanczos`        — Lanczos-with-full-reorthogonalization driver
//!                      used by `sparse_fiedler` to converge on `λ_2(L)`
//!                      even when `λ_2 ≪ λ_max` (path-like topologies
//!                      where shifted power iteration collapses to 0).
