...
def update(port, image, boot_timeout=20):
    import serial
    ...
    with serial.Serial(port, 115200, timeout=1) as s:
        ...
        while sent < len(data):
            chunk = data[sent:sent+block]; s.write(chunk); s.flush(); sent += len(chunk)
            if sent < len(data):
        ...
        # The board restarts; collect the boot report and the commit/rollback decision.
