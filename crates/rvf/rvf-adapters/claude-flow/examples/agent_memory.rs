...
fn main() -> Result<(), Box<dyn std::error::Error>> {  
    ...
    // Ingest three memories with deterministic embeddings (seeded pattern)  
    let memories = [  
        ("user-theme", "dark-mode", "ui", vec![0.1, 0.2, 0.3, 0.4]),  
        ("user-lang", "english", "profile", vec![0.2, 0.3, 0.4, 0.5]),  
        ("project", "rvf-adapter", "work", vec![0.3, 0.4, 0.5, 0.6]),  
    ];  
  
    for (i, (key, value, ns, embedding)) in memories.iter().enumerate() {  
        let id = store.ingest_memory(key, value, Some(ns), embedding)?;  
        println!("Ingested memory {}: id={}, key={}, value={}", i + 1, id, key, value);  
        // Record a witness action for audit trail  
        store  
            .witness()  
            .record_ingest(key, value, *ns)  
            .unwrap();  
    }  
    ...
    // ------------------------------------------------------------------  
    store.close()?;  
    let config2 = ClaudeFlowConfig::new(tmp_dir.path(), "agent-demo")  
        .with_dimension(4)  
        .with_namespace("session-42");  
    let store_reopened = RvfMemoryStore::open(config2)?;  
    ...
    // Verify we can retrieve the same memories  
    ...
    // ------------------------------------------------------------------  
    let entries: Vec<_> = store_reopened.witness().entries().take(3).collect();  
