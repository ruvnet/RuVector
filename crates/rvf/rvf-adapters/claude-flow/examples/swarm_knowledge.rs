...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (agent_id, key, ns)) in agents.iter().enumerate() {
        ...
        let id = store.ingest_memory(
            key,
            &format!("Shared by {}", agent_id),
            Some(*ns),
            &embedding,
        )?;
        println!(
            "Agent {} shared knowledge {}: id={}, key={}",
            agent_id,
            i + 1,
            id,
            key
        ...
        store
            .coordination()
            .record_state(agent_id, "status", "active")?;
        store
            .coordination()
            .record_state(agent_id, "last_shared", key)?;

        // Record witness for knowledge sharing
        store
            .witness()
            .record_action("share-knowledge", &[agent_id, key])?;

        // Store learning pattern for this type of knowledge
        let pattern_type = if *ns == "coordination" {
            "convergent"
        } else {
            "divergent"
        };
    ...
    store
        .coordination()
        .record_state("swarm", "topology", "mesh")?;
    store
        .coordination()
        .record_state("swarm", "coordinator", "agent-coordinator")?;
    store
        .witness()
        .record_action("topology-change", &["mesh"])?;
    ...
    let all_states = store.coordination_ref().get_all_states();
    ...
    let top_patterns = store.learning_ref().get_top_patterns(3);
    ...
    // ------------------------------------------------------------------
    let witness_entries: Vec<_> = store.witness_ref().entries().take(5).collect();
