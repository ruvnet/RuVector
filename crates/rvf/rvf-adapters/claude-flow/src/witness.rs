...
/// A witness entry enriched with Claude-Flow context.  
...
impl WitnessWriter {  
    ...
    pub fn create(config: &ClaudeFlowConfig) -> Result<Self, ClaudeFlowError> {  
        config.ensure_dirs().map_err(|e| ClaudeFlowError::Io(e.to_string()))?;  
    ...
    pub fn open(config: &ClaudeFlowConfig) -> Result<Self, ClaudeFlowError> {  
        ...
        let data = std::fs::read(&path).map_err(|e| ClaudeFlowError::Io(e.to_string()))?;  
        let entries = verify_witness_chain(&data).map_err(ClaudeFlowError::Witness)?;  
    ...
    pub fn record_search(&mut self, query_k: usize) -> Result<(), ClaudeFlowError> {  
        let payload = query_k.to_string();  
        self.record_with_type(types::SEARCH_MEMORY, "search_memory", &[&payload])  
    }  
  
    /// Record a session event.  
    pub fn record_session(&mut self, event: &str) -> Result<(), ClaudeFlowError> {  
    ...
    pub fn flush(&mut self) -> Result<(), ClaudeFlowError> {  
        ...
        file.write_all(&chain_bytes)  
            .map_err(|e| ClaudeFlowError::Io(e.to_string()))?;  
        file.sync_all()  
            .map_err(|e| ClaudeFlowError::Io(e_string()))?;  
        Ok(())  
    }  
  
    /// Verify the integrity of the loaded chain.  
    pub fn verify(&self) -> Result<(), ClaudeFlowError> {  
        let chain_bytes = create_witness_chain(&self.entries);  
        verify_witness_chain(&chain_bytes)  
            .map(|_| ())  
            .map_err(ClaudeFlowError::Witness)  
    }  
...
mod tests {  
    ...
    fn ingest_and_search_events() {  
        ...
        assert_eq!(w.len(), 3);  
        let types: Vec<_> = w.entries().map(|e| e.witness_type).collect();  
        assert_eq!(types, [types::INGEST_MEMORY, types::SEARCH_MEMORY, types::SESSION_EVENT]);  
    }  
