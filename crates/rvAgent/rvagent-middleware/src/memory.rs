...
// We use a simple hex encoding inline to avoid an extra dependency.
mod hex {
...
impl Middleware for MemoryMiddleware {
    ...
    fn before_agent(
        ...
        let contents = if let Some(preloaded) = &self.preloaded {
            // Use preloaded contents (filtered by validation)
            preloaded
                .iter()
            ...
            // In production, would load from backend here.
            // Return empty for now — actual loading delegated to runtime.
            HashMap::new()
        };
...
mod tests {
    ...
    impl ModelHandler for PassthroughHandler {
        fn call(&self, request: ModelRequest) -> ModelResponse {
            ModelResponse::text(
                request.system_message.unwrap_or_default(),
            )
        }
    }
    ...
    fn test_security_policy_warn_untrusted() {
        ...
        // Hash mismatch still loads (with warning)
    ...
    fn test_wrap_model_call_no_memory() {
        ...
        // No memory -> no system message modification
