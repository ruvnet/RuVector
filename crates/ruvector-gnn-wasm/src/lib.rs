...
mod wasm_bindings {
    ...
    pub fn differentiable_search(
        ...
        let candidates: Vec<Vec<f32>> =
            serde_wasm_bindgen::from_value(candidate_embeddings).map_err(|e| {
                JsValue::from_str(&format!("Failed to parse candidate embeddings: {}", e))
            })?;
    ...
    pub fn hierarchical_forward(
        ...
        let embeddings: Vec<Vec<Vec<f32>>> =
            serde_wasm_bindgen::from_value(layer_embeddings).map_err(|e| {
                JsValue::from_str(&format!("Failed to parse layer embeddings: {}", e))
            })?;

        let core_layers: Vec<RuvectorLayer> =
            gnn_layers.iter().map(|l| l.inner.clone()).collect();
...
// Re-export all WASM bindings when targeting wasm32
#[cfg(target_arch = "wasm32")]
pub use wasm_bindings::*;

// Non-WASM stub: provide a minimal public API for native compilation
