...
pub fn run_lsm_streaming(dataset: &Dataset, k: usize) -> anyhow::Result<Vec<(f64, f64, f64)>> {
    ...
    for &cp in &checkpoints {
        ...
        // Measure recall at this fill level
        let total_recall: f64 = dataset
            .queries
            .iter()
            .take(50) // small query set for speed
            .enumerate()
            .map(|(qi, q)| {
                let res = idx.search(q, k.max(10));
                let ids: Vec<u64> = res.iter().map(|&(id, _)| id).collect();
                dataset.recall_at_k(qi, &ids, 10)
            })
            .sum::<f64>()
            / 50.0;

        let fill_pct = inserted as f64 / n as f64 * 100.0;
        results.push((fill_pct, total_recall, idx.memory_bytes() as f64 / (1024.0 * 1024.0)));
    }
