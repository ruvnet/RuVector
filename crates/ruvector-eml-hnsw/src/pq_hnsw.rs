...
impl PqEmlHnsw {
    ...
    pub fn search_with_rerank(
        ...
        // Do NOT pre-truncate by PQ distance: the whole point of pairing
        // PQ + exact rerank is that we use PQ as the graph-traversal kernel
        // and exact as the final ranker. Truncating by PQ distance before
        // rerank re-introduces the approximation error we are trying to
