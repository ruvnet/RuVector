...
fn main() {
    ...
    let edges: Vec<_> = lines.map(|line| {
        let p: Vec<_> = line.split_whitespace().collect();
        (p[0].parse::<usize>().unwrap(), p[1].parse::<usize>().unwrap(), p[2].parse::<f64>().unwrap())
    }).collect();
    for iteration in 0..3 {
        ...
        let witness: f64 = edges.iter().filter(|(u,v,_)| side.contains(u) != side.contains(v)).map(|e| e.2).sum();
