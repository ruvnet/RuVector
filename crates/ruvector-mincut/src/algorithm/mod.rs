...
impl DynamicMinCut {
    ...
    pub fn update_edge(&mut self, u: VertexId, v: VertexId, weight: Weight) -> Result<f64> {
        ...
        let Some(old) = old else {
            return self.insert_edge(u, v, weight);
        };
        let start = PortableInstant::now();
    ...
    fn record_batch(&self, insertion: bool, count: usize, elapsed_us: f64) {
        ...
        if insertion {
            stats.insertions += count as u64;
        } else {
            stats.deletions += count as u64;
        }
        stats.avg_update_time_us =
            (stats.avg_update_time_us * old_count + elapsed_us) / (old_count + count as f64);
    }
