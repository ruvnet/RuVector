...
fn check(solver: &DynamicMinCut) {
    ...
    assert_eq!(
        s.union(&t).copied().collect::<HashSet<_>>(),
        vertices.iter().copied().collect()
    );
    if vertices.len() >= 2 {
        assert!(!s.is_empty() && !t.is_empty());
        let crossing: HashSet<_> = edges
            .iter()
            .filter(|e| s.contains(&e.source) != s.contains(&e.target))
            .map(|e| e.id)
            .collect();
        let witness = result.cut_edges.unwrap();
        assert_eq!(
            witness.iter().map(|e| e.id).collect::<HashSet<_>>(),
            crossing
        );
...
fn mixed_updates_match_oracle_after_every_mutation() {
    ...
    for u in 0..6 {
        for v in u + 1..6 {
            solver
                .insert_edge(u * 100 + 7, v * 100 + 7, ((u + v) % 5) as f64 / 2.0)
                .unwrap();
            check(&solver);
        }
    }
...
fn partitions_are_stable_across_input_order() {
    ...
    let a = MinCutBuilder::new()
        .with_edges(edges.clone())
        .build()
        .unwrap();
    let b = MinCutBuilder::new()
        .with_edges(edges.into_iter().rev().collect())
        .build()
        .unwrap();
...
fn batches_validate_before_mutating_and_solve_once() {
    ...
    assert_eq!(solver.stats().insertions, 0);
    solver
        .insert_edges(&[(0, 1, 2.0), (1, 2, 3.0), (2, 0, 4.0)])
        .unwrap();
    ...
    assert!(solver
        .insert_edges(&[(2, 3, 1.0), (3, 4, f64::NAN)])
        .is_err());
