...
fn main() {
    for n in [32u64, 128] {
        ...
        println!(
            "tree_build n={n} iterations=20 mean_us={:.3}",
    ...
    let inserts: Vec<_> = (0..20)
        .flat_map(|u| (u + 2..40).map(move |v| (u, v, 10.0)))
        .collect();
    ...
    println!(
        "preserving_inserts n=64 operations={} mean_us={:.3}",
        inserts.len(),
        start.elapsed().as_secs_f64() * 1e6 / inserts.len() as f64
    );
    let start = Instant::now();
    for _ in 0..1000 {
        black_box(solver.min_cut());
    }
    println!(
        "full_result n=64 iterations=1000 mean_us={:.3}",
    ...
    println!(
        "deletions n=64 operations=20 mean_us={:.3}",
        start.elapsed().as_secs_f64() * 1e6 / 20.0
    );
}
