...
fn create_complex_dag(size: usize) -> QueryDag {
    ...
    for i in 0..size {
        let op_type = match i % 4 {
            0 => OperatorType::SeqScan { table: format!("t{}", i) },
            1 => OperatorType::HnswScan { index: format!("idx{}", i), dim: 128 },
            2 => OperatorType::HashJoin { key: format!("key{}", i) },
            _ => OperatorType::Filter { condition: format!("col{} > {}", i, i * 10) },
        };
...
fn bench_topological_sort(c: &mut Criterion) {
    ...
    for size in [10, 100, 500, 1000] {
        ...
        group.bench_with_input(
            ...
            &dag,
            |b, dag| b.iter(|| dag.topological_sort()),
        );
    }
...
fn bench_attention(c: &mut Criterion) {
    ...
    group.bench_function("topological_100", |b| {
...
fn bench_lora_adaptation(c: &mut Criterion) {
    ...
    group.bench_function("adapt_256", |b| {
        b.iter(|| lora.adapt(&gradient, 0.1))
    });
...
fn bench_reasoning_bank(c: &mut Criterion) {
    ...
    group.bench_function("store_pattern", |b| {
        ...
        b.iter(|| {
            let pattern: Vec<f32> = (0..256).map(|j| {
                ((counter * 256 + j) as f32 / 1000.0).cos()
            }).collect();
...
fn bench_dag_depths(c: &mut Criterion) {
    ...
    for size in [10, 100, 500] {
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(size),
            &dag,
            |b, dag| b.iter(|| dag.compute_depths()),
        );
    }
...
fn bench_dag_serialization(c: &mut Criterion) {
    ...
    group.bench_function("from_json", |b| {
        b.iter(|| QueryDag::from_json(&json))
    });
