//! Nightly follow-up benchmark (2026-09-20), re-running ADR-345's exact
//! rejected hypothesis unmodified — same corpus, same seed, same acceptance
//! thresholds as `mincut_gated_forgetting_bench.rs` — with the sole change
//! being `MincutGatedForgetting::{soft,hard}` swapped for
//! `MincutGatedForgetting::{soft_canonical,hard_canonical}`
//! (`MincutBackend::Canonical`, i.e.
//! `ruvector_mincut::SourceAnchoredMinCut` instead of
//! `RuVectorGraphAnalyzer`). Per the nightly process's "don't move the
//! goalposts" rule (ADR-345's own "Next Research" item 3): if this changes
//! the accept/reject outcome, that is itself the evidence.
//!
...
//! accesses), and a k-NN (k=5, cosine >= 0.05) similarity graph feeding the
//! mincut backend under test,
//!
...
//! baseline's on the same corpus, and (b) 100% tamper-detection across 20
...
fn run_tamper_trials(seed: u64) -> (usize, usize) {
    ...
    for trial in 0..N_TAMPER_TRIALS {
        ...
        let policy =
            MincutGatedForgetting::soft_canonical(CoherenceWeights::default(), STRUCTURAL_BONUS);
...
fn main() {
    ...
    println!("╔══════════════════════════════════════════════════════════════════╗");
    println!("║  ruvector-agent-memory — Mincut-Gated Forgetting, canonical backend║");
    println!("║  (2026-09-20 nightly follow-up to ADR-345)                        ║");
    println!("╚══════════════════════════════════════════════════════════════════╝\n");
    ...
    println!("Dataset (unchanged from ADR-345)");
    ...
    let soft = MincutGatedForgetting::soft_canonical(CoherenceWeights::default(), STRUCTURAL_BONUS);
    let hard = MincutGatedForgetting::hard_canonical(CoherenceWeights::default(), PROTECT_FRACTION);
    ...
    println!("Acceptance test (thresholds unchanged from ADR-345)");
    ...
    if all_pass {
        println!("=> ACCEPT: mincut-gated forgetting (canonical backend) protects structural bridges at no material recall or witness-integrity cost.");
    } else {
