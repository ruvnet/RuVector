...
- ✗ **DiskANN / Vamana motif index** (commits 13, reverted commit 14) — attempted as the named follow-up to AC-2's precision@5 = 0.60 gap. Agent shipped a Vamana index + 605-window corpus but measured `precision@5 = 0.551` — *worse* than brute-force 0.60 on the same data. Diagnosis: the bottleneck isn't the index choice but the corpus's `distinct_labels = 4` / `max_label_share = 0.49` structure. At that label granularity the theoretical ceiling on precision@5 is not much above random, so no index helps. **→ next:** expand the motif-corpus label vocabulary (more stimulus protocols / longer simulations / per-module label granularity) before re-attempting the ANN swap.
...
The discoveries form a pattern: every "next lever named in the ADR" ultimately required an empirical test. Three of the five pre-measurement diagnoses in this branch's ADR §16 history proved wrong when tested (items 7, 8, 9). **The successful lever (item 6, adaptive cadence) was an orthogonal axis — schedule of detection, not algorithm of detection.** That insight is the deepest lesson the branch has to offer and is probably generalisable to other "detect-and-act" pipelines.
