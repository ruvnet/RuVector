...
export async function initOnnxEmbedder(config: OnnxEmbedderConfig = {}): Promise<boolean> {
  ...
  loadPromise = (async () => {
    try {
      ...
      const bgJsUrl = pathToFileURL(bgJsPath).href;
      ...
      // `WebAssembly` is a JS global available in every Node/browser runtime,
      // but isn't in the `lib: ["ES2020"]` TS typings, hence the cast.
      const wasmRuntime = (globalThis as any).WebAssembly as {
        instantiate(bytes: Uint8Array, imports: Record<string, unknown>): Promise<{ instance: { exports: any } }>;
      };
