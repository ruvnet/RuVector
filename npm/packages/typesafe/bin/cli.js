...
/**
 * `typesafe` CLI entry (plain Node, no transpile). Every subcommand except
 * `bench` is handled by ../dist/cli/main.js. `bench` lives here because it must
 * load bench/run.mjs with a real dynamic import() — one that a CommonJS build
 * step cannot rewrite into require(). No child_process, no network.
 */
...
async function runBench(argv) {
  ...
  // bench/run.mjs exports `main(argv, deps)` (older drafts exported `run`); it
  // only self-runs when invoked directly, so we must call it explicitly here.
  ...
  // No callable entry: importing already ran any top-level side effects.
  process.exit(0);
}
...
function loadMain() {
  try {
    ...
    process.stderr.write('Failed to load @ruvector/typesafe CLI. Run: npm run build\n');
